...
for i, color in enumerate(['default', *style.colors]):
    if i < 5:
        a = ttk.Radiobutton(top_frame, text=color, bootstyle=color + 'outline-toolbutton', width=12, value=i)
    else:
        a = ttk.Radiobutton(bot_frame, text=color, bootstyle=color + 'outline-toolbutton', width=12, value=i)
    a.pack(side=tk.LEFT, padx=3, pady=10)
a = ttk.Radiobutton(bot_frame, text='disabled', width=12, bootstyle="outline-toolbutton", state=tk.DISABLED)
a.pack(side=tk.LEFT, padx=3, pady=10)
