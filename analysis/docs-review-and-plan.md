...
- [x] **10C. `docs/guides/app-structure.md`** (2026-04-30). Rewrote as progressive architecture guide. Key fixes: localization example removed (incorrect message-id lookup pattern); added `show_nav=False` note; documented `Signal` constructor and `subscribe` callback semantics; verified `PackFrame`/`GridFrame` params; added `on_close` lifecycle + macOS caveat; added `GridFrame` form example. **Code-bug noted**: `App(theme="cosmo", settings={"light_theme": "cosmo"})` crashes with `ThemeError` because alias registration happens before settings overrides land during `tkinter.Tk.__init__`.
- [x] **10D. `docs/guides/reactivity.md`** (2026-04-30). Rewrote with React/Vue/SwiftUI framing. Key fixes: `unsubscribe(callback)` silently no-ops — real API is `unsubscribe(funcid)`; `signal.value` property doesn't exist; `subscribe(immediate=True)` and `unsubscribe_all()` were undocumented; strict type enforcement on `set()` not mentioned; `Signal.from_variable()` / `.var` undocumented. Added: "problem signals solve" motivating section, derived-signal pitfall (weakref — map() result can be GC'd), `Signal.from_variable()` interop section, pitfalls list. **Capability page drift noted**: `docs/capabilities/signals/signals.md` has same `unsubscribe(callback)` bug. **Code-bug noted**: `Signal.map()` uses weakref so one-liner `textvariable=sig.map(fn)` may stop updating after GC; `signal.value` property would be a useful ergonomic addition.
- [x] **10E-toolbars.** `docs/guides/toolbars.md` (2026-04-30). AppShell-first reorder; hamburger/spacer pre-population explained; `frameless=True` as canonical custom-titlebar; keyboard access section added; confirmed no overflow API. Drift caught: `TextEntry(placeholder=)` not real, `Shortcuts.bind(...)` not real (real API is `register`+`bind_to`).
- [x] **10E-forms.** `docs/guides/forms.md` (2026-04-30). Key fixes: `field_textsignal(key)` returns None (correct path is `form.field(key).signal`); `compare` rule type doesn't exist; `<<Validated>>` → `<<Validate>>`; `stringLength` uses `min`/`max` not `min_length`/`max_length`; trigger gotcha documented (blur-triggered rules skip `validate()`). Added async validation pattern, dirty-state/reset section.
- [x] **10E-tables.** `docs/guides/tables-and-lists.md` (2026-04-30). Added widget-choice matrix, selection matrix, pagination-mode matrix. Drift fixed: `ttk.ListView` now public; `row_factory` subclassing contract is `update_data(record)` not `render(record)`; `<<ItemClick>>` payload is the record dict directly (not `event.data["record"]`); column dict keys are `key`/`text`/`width`/`minwidth` (not `label`). Code bugs noted: `ListView` docstring event-payload docs wrong; internal `MemoryDataSource` / `DataSourceProtocol` duplicated with different method names.
- [x] **10F-styling.** `docs/guides/styling.md` (2026-04-30). Fictional surface tokens removed (`titlebar`, bracketed variants). `CheckButton(variant="toggle")` removed (correct widgets are `CheckToggle`/`Switch`). Added `density` as a first-class section (was missing). `text` button variant added. Color-modifier deep-dive compressed to pointer. Verified `Style.configure/map/lookup/theme_names` all real.
