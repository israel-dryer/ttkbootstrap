...
class TTkIconButtonTextStyle(StyleBuilder):
    ...
    def invoke(self, token: str, **extras):
        """Create the text icon button style"""

        background, style = self.theme.get_background_style(token, 'Text.Icon.TButton', **extras)
        if self.theme.has_style(style):
            return style

        token = "surface" if token == "default" else token
        colors = self.theme.get_color_states(token, "text", background)

        # state images
        normal_img = self.theme.recolor_state_image('button-text.png', colors.normal.color)
