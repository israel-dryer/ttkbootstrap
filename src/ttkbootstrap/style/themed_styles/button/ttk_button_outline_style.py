...
class TTkButtonOutlineStyle(StyleBuilder):
    ...
    def invoke(self, token: str, **extras):
        ...
        colors = self.theme.get_color_states(base_color, "outline", background)

        # state images
        normal_img = self.theme.recolor_state_image('button-outline.png', colors.foreground)
        ...
        self.theme.map(
            style,
            foreground=[('disabled', colors.foreground_disabled), ('hover', colors.foreground_hover)],
            focuscolor=[('focus hover', colors.foreground_hover), ("hover", colors.foreground_hover), ('focus', colors.foreground)]
        )
