...
class SwitchOptions(TypedDict, total=False):
    """Typed dictionary for supported ttk switch options."""
...
class Switch(StyleMixin, BaseMixin, BackgroundMixin):
    """
    A styled `Switch` widget with theme-aware styling, value binding, and event callbacks.

    Args:
        master (Optional[Misc]): Parent widget.
        text (Optional[str]): The label text of the switch.
        value: Literal[-1, 0, 1] The initial value of the switch.
        color (StyleColor): Named style color for theming.
        on_click (Optional[Callable]): Callback function when the switch is clicked.
        on_value_changed (Optional[Callable]): Callback function when the value changes.
        **kwargs (Unpack[CheckButtonOptions]): Additional keyword options passed to ttk.Switch.

    Example:
        ```python
        Switch(root, text="Enable Feature", value=True, color="success", on_value_changed=on_toggle)
        ```
    """
    ...
    def widget(self) -> Misc:
        """Return the internal switch widget."""
        return self._widget

    @property
    def text_variable(self) -> StringVar:
        """Return the StringVar linked to the switch label."""
        return self._text_variable

    @property
    def variable(self):
        """Return the Variable linked to the switch value."""
    ...
    def enabled(self) -> bool:
        """Return True if the switch is enabled; otherwise False."""
    ...
    def on_value_changed(self):
        """Get or set the callback function triggered when the switch value changes."""
    ...
    @property
    def cursor(self) -> str:
        """Get or set the mouse cursor when hovering over the switch."""
    ...
    @property
    def width(self) -> int:
        """Get or set the width of the switch in text units."""
    ...
    def invoke(self):
        """
        Programmatically trigger the switch's click action.
