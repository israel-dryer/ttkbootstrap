...
class RadioButtonToggleOptions(TypedDict, total=False):
    """Typed dictionary for supported ttk radiobutton options."""
...
class RadioButtonToggle(StyleMixin, BaseMixin, IconMixin, BackgroundMixin):
    """
    A styled `RadioButtonToggle` widget with theme-aware styling, value binding, and event callbacks.
    ...
    Args:
        ...
        on_click (Optional[Callable]): Callback function when the radiobutton is clicked.
    ...
    Example:
        ```python
        RadioButtonToggle(root, text="Generator", group="options", value="gen", color="success", on_value_changed=on_toggle)
        ```
    """
    ...
    def __init__(
        ...
        **kwargs: Unpack[RadioButtonToggleOptions]
    ):
        kw = dict(kwargs)
        self._master = master
        self._group = group
    ...
    def widget(self) -> Misc:
        """Return the internal radiobutton widget."""
        return self._widget

    @property
    def text_variable(self) -> StringVar:
        """Return the StringVar linked to the radiobutton label."""
        return self._text_variable

    @property
    def variable(self):
        """Return the Variable linked to the radiobutton value."""
    ...
    @property
    def value(self):
        """Get or set the radiobutton value."""
    ...
    def enabled(self) -> bool:
        """Return True if the radiobutton is enabled; otherwise False."""
    ...
    def on_click(self) -> Optional[Callable]:
        """Get or set the command function triggered on radiobutton click."""
    ...
    def on_value_changed(self):
        """Get or set the callback function triggered when the radiobutton value changes."""
    ...
    @property
    def cursor(self) -> str:
        """Get or set the mouse cursor when hovering over the radiobutton."""
    ...
    @property
    def width(self) -> int:
        """Get or set the width of the radiobutton in text units."""
