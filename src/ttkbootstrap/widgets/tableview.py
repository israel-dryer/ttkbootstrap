...
from tkinter import IntVar, StringVar, Menu, font
from tkinter.ttk import Treeview, Frame, Scrollbar, Button, Label, Entry, Separator
from ttkbootstrap.style import Style

from ttkbootstrap.utility import scale_size
from ttkbootstrap.constants import DEFAULT, HEADINGS, W, END, E, CENTER, BOTH, YES
from ttkbootstrap.constants import TOP, EXTENDED, LEFT, VERTICAL, Y, RIGHT, HORIZONTAL, X, N
...
class TableColumn:
    ...
    def __init__(
        ...
        self.view: Treeview = tableview.view
        self.view.column(
            self._cid,
            width=width,
            minwidth=minwidth,
...
class Tableview(Frame):
    """A class built on the `Treeview` widget for arranging data in
    ...
    def __init__(
        ...
        self._viewdata = []
        self._rowindex = IntVar(value=0)
        self._pageindex = IntVar(value=1)
        self._pagelimit = IntVar(value=0)
        self._height = height
        self._pagesize = IntVar(value=pagesize)
    ...
    def autofit_columns(self):
        ...
        pad = scale_size(self, 20)
        col_widths = []

        # measure header sizes
        for col in self.tablecolumns:
            width = f.measure(f"{col._headertext} {DOWN_ARROW}") + pad
    ...
    def _column_sort_header_update(self, cid):
        ...
        column: TableColumn = self.cidmap.get(int(cid))
        arrow = UP_ARROW if column.columnsort == ASCENDING else DOWN_ARROW
    ...
    def _build_search_frame(self):
        ...
        Label(frame, text=MessageCatalog.translate("Search")).pack(side=LEFT, padx=5)
        searchterm = Entry(frame, textvariable=self._searchcriteria)
    ...
    def _build_pagination_frame(self):
        ...
        Separator(pageframe, orient=VERTICAL).pack(side=RIGHT, padx=10)
        ...
        lbl = Label(pageframe, textvariable=self._pagelimit)
        lbl.pack(side=RIGHT, padx=(0, 5))
        Label(pageframe, text=MessageCatalog.translate("of")).pack(side=RIGHT, padx=(5, 0))

        index = Entry(pageframe, textvariable=self._pageindex, width=4)
        index.pack(side=RIGHT)
        index.bind("<Return>", self.goto_page, "+")
        index.bind("<KP_Enter>", self.goto_page, "+")

        Label(pageframe, text=MessageCatalog.translate("Page")).pack(side=RIGHT, padx=5)
...
class TableCellRightClickMenu(Menu):
    ...
    def __init__(self, master: Tableview):
        ...
        self.view: Treeview = master.view
        self.cid = None
        self.iid = None
        ...
        sort_menu = Menu(self, tearoff=False)
        ...
        filter_menu = Menu(self, tearoff=False)
        ...
        export_menu = Menu(self, tearoff=False)
        ...
        move_menu = Menu(self, tearoff=False)
        ...
        align_menu = Menu(self, tearoff=False)
...
class TableHeaderRightClickMenu(Menu):
    ...
    def _build_show_menu(self):
        ...
        if self._show_menu is not None:
            ...
            self._show_menu = Menu(self, tearoff=False)
