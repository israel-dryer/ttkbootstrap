...
# Partials that a page folds in *underneath* another family's partial, as a
# named variant section (e.g. "Toolbutton variant" on the Button page). Their
# subsections are rendered one heading level deeper (`^` rather than `~`) so
# they nest under that heading instead of repeating the parent's headings at the
# same level. A partial that is the *only* include on its page (calendar,
# floodgauge) is not nested and keeps `~`.
#
# This must match the pages; `tests/test_style_reference.py` enforces that by
# deriving the same set from the `.. include::` order in `docs/reference/api/`.
NESTED_PARTIALS = frozenset({"toggle", "toolbutton"})
...
def render_style_partial(style, family):
    ...
    uc = "^" if family in NESTED_PARTIALS else "~"
    parts = [
        ".. Generated by tools/generate_style_reference.py -- do not edit by hand.",
        "",
    ] + _style_body_parts(style, family, uc)
    return "\n".join(parts)
