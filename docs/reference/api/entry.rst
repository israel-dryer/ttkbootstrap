...
.. note::
   ...
   Python's standard library documents ``ttk.Entry`` only briefly. The canonical
   upstream source is the
   `Tk ttk::entry manual page <https://www.tcl-lang.org/man/tcl8.6/TkCmd/ttk_entry.htm>`__
   (Tcl 8.6).
...
.. list-table::
   ...
   * - ``bootstyle``
     ...
     - **Constructor keyword.** An accent color, optionally with a variant. See
       :ref:`Styling options <entry-styling>` for the available styles.
   * - ``textvariable``
     - ``Variable``
...
This section is for changing how the entry *looks*. Define a style with
``style.configure(...)`` (and ``style.map(...)`` for per-state colors), then
apply it with ``Entry(style=...)``.

.. include:: /reference/api/_style/entry.rst

Shared capabilities
-------------------

``Entry`` also has the methods every widget inherits — configuration,
...
- :doc:`Entry catalog page </widgets/entry>` — usage, screenshots, and examples.
- `Tk ttk::entry manual page <https://www.tcl-lang.org/man/tcl8.6/TkCmd/ttk_entry.htm>`__
