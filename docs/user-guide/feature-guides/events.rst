...
Every tkinter app is event-driven: your code runs in callbacks that fire in
response to clicks, keystrokes, and higher-level notifications. The
:doc:`Events & callbacks </user-guide/foundations/events-and-callbacks>`
foundations page covers the essentials — ``command``, ``bind``, and ``after``.
This guide goes deeper into the binding system: how an event finds its
callbacks, how to share and stop bindings, and how to define and dispatch your
own events.
